...
- 1. 어떤 노드가 N을 기준으로 왼쪽 서브 트리 노드의 모든 키 값은 노드 N의 키 값보다 작아야함
...
- 4. 노드를 삽입하기 쉬움 
- 다음 그림은 이진 검색 트리의 생성 예시 입니다.
- ![이미지](/image.png)
- ## 4.이진 검색 트리
- 검색 과정은 다음과 같습니다
- 1. 루트에서 시작합니다
- 2. 검색 값을 루트와 비교합니다. 루트보다 자긍면 왼쪽에 대해 재귀하고 크다면 오른쪽으로 재귀합니다
- 3. 일치하는 값을 찾을 때까지 절차를 반복합니다
- 4. 검색 값이 없으면 null을 반환합니다
- 다음 그림은 60을 찾는 과정 입니다
-  ![이미지](/20251127_114752.jpg)
- 삽입 과정은 다음과 같습니다
- Root에서 시작합니다
- 삽입 값을 루트와 비교합니다 루트보다 작으면 왼쪽으로 재귀하고 크다면 오른쪽으로 재귀합니다
- 리프 노드에 도달한 후 노드보다 크다면 
- 삭제할 노드가 리프 노드인 경우는 검색 후 바로 삭제하면 됩니다. 
- 삭제할 노드에 자식이 하나만 있는 경우는 노드를 삭제하고, 자식 노드를 삭제된 노드의 부모에 직접 연결합니다
- 3. 삭제할 노두애 자식이 둘 있는 경우는 successor 노드를 찾는 과정이 추가됩니다.
- successor 노드란 삭제할 노드의 값 보다 크면서 가장 작은 키를 가진 노드입니다
- 즉 오른쪽 서브 트리 중에서 가장 작은 값을 의미 합니다
- * 삭제 과정은 다음과 같습니다
- 1. 삭제할 노드를 찾습니다
- 2. 삭제할 노드의 successor 노드를 찾습니다
- 3. 삭제할 노듸와 successor 노드의 값을 바꿉니다
- 4. successor 노드를 삭제합니다
- # 해시법
- ## 1.해시법이란? 
- 해시법은 데이터를 저장할 인덱스를 해시 함수로 계산하여, 데이터의 검색, 추가, 삭제를 효율적으로 수행하는 자료 구조 입니다
- 해시 함수를 이용해 키 값을 고정된 길이의 해시 값으로 변환하고, 이 해시 값을 배열의 인덱스로 사용하여 데이터를 저장하는 방식 입니다.
- 이 과정에서 여러 키가 동일한 해시 값으로 변환되는 해시 충돌이 발생할수 있으며, 이를 해결하기 위해 체인법이나 개방 주소 법 같은 방법들이 사용됩니다.
- [핵심요소]
- 해시 함수: 임이의 길이를 가진 데이터를 고정된 길이의 값으로 변환하는 함수
- 해시 데이터: 해시 함수를 통해 얻은 해시 값을 인덱스로 사용하여 데이터를 저장하는 배열 기반의 자료 구조입니다
- 해시 충돌: 서로 다은 키 값이 해시 함수를 통해 같은 해시 값으로 변환되어 같은 인덱스에 저장하는 현상입니다
- [충돌이 왜 발생하나?]
- git 등에서 쓰는 해시는 사실상 충돌이 없다고 간주할 만큼 강력한 암호학적 해시입니다
- 이에 반해 자료구조 해시는 빠른 접근을 위해 설계된 약한 해시입니다
- 따라서 git 같은 시스템에서는 사실상 충돌이 거의 발생하지 않지만, 자료구조에서는 충돌이 자주 발생합니다
- 1. git의 해시는 암호학적 해시
- 입력이 조금만 달라도 완전히 다른 해시가 나옴
- 임의로 충돌을 만들기는 거의 불가능
- [해시 값을 사용하는데 왜 충돌이 발생하나?]
- 2. 자료구조의 해시는 빠르고 단순하기 위해 만든 해시함수
- 자료구조에서 사용하는 해시는 목적이 전혀 다름
- 
- 3. 체인 법
- 해시 테이블의 각 버킷이 리스트를 가지고 있어서,충돌이 발생하면 그 버킷의 리스트에 노드를 추가하는 방식입니다
- 장점
- 구현이 매우 간단
- 테이블이 거의 가득 차도 삽입 가능
- 삭제가 쉬움
- 단점
- 충돌이 많아지면 리스트 길이가 길어져 검색시간이 0 으로 악화
- 리스트를 위한 추가 메모리 사용
- 포인터 사용으로 메모리에 locality가 떨어져 느려질 수 있음
- 
- 
- 
- 
